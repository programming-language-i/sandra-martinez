...
R//:Concurrencia. ya que solo se usa un solo nucleo
2. Nombrar dos diferencias entre un proceso y un hilo *(memoria, costo, fallo)*.
R//: Memoria: tiene su propio espacio  y en hilo comparte la memoria.
3. ¿En qué estado del ciclo de vida está un hilo que ejecuta `time.sleep(2)`? ¿Qué devuelve `is_alive()`?
R//:esta en un estado bloqueado donde se devuelve a tru porque el hilo solo esta pausado 
4. ¿Qué protege el GIL? ¿Evita las condiciones de carrera en los datos del programa?
R//:protege el conteo de referencias de los objetos, que no es seguro entre hilos.
5. ¿Por qué 4 hilos que duermen 1 s cada uno tardan ~1 s en total y no ~0,25 s?
R//:mientras un hilo duerme no consume ni bloquea a los demás, así que los 4 sleeps ocurren de forma concurrente.
6. Al crear un hilo por herencia, ¿qué método se sobrescribe y cuál nunca? ¿Por qué?
R//:solo se puede sobreescribir el metodo run() pero nunca el start()
7. ¿Qué pasa con un hilo daemon cuando termina el hilo principal? ¿Qué **no** se ejecuta?
R//:el hilo principal terminan, el hilo daemon se corta sin aviso ni espera. no lo esperalo para ejecutarse otra vez
ya lo que queda pendiente lo que puede estar en el hilo  hilo 
8. Completar la regla del curso: *hilos para , procesos para*.
R//: los hilos se usan para esperar y los procesos para calcularS
